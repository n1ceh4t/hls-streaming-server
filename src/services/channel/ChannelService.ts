...
export class ChannelService {
  ...
  private concatProgressionTrackers: Map<string, {
    ...
    lastPreGeneratedTransition?: string; // Track which transition we pre-generated for (e.g., "318->319")
  }> = new Map();
  ...
  private startConcatProgressionTracking(
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        // CRITICAL: Check if FFmpeg is still running - if not, restart stream
        // This handles cases where FFmpeg crashes due to missing/corrupted bumper files
        const isFFmpegActive = this.ffmpegEngine.isActive(channelId);
        if (!isFFmpegActive && channel.isStreaming()) {
          logger.warn(
            { channelId, state: channel.getState() },
            'FFmpeg process stopped unexpectedly - attempting to restart stream'
          );
          
          // Check if there are active viewers before restarting
          const hasViewers = await this.hasActiveViewers(channelId);
          if (hasViewers) {
            try {
              // Get current position from progression tracker
              const tracker = this.concatProgressionTrackers.get(channelId);
              const currentIndex = tracker?.currentFileIndex ?? startIndex;
              
              logger.info(
                { channelId, currentIndex, hasViewers },
                'Restarting stream after unexpected FFmpeg stop (viewers present)'
              );
              
              // Restart the channel from current position
              await this.startChannel(channelId, currentIndex, true);
            } catch (restartError) {
              logger.error(
                { channelId, error: restartError },
                'Failed to restart stream after FFmpeg crash'
              );
            }
          } else {
            logger.info(
              { channelId },
              'FFmpeg stopped but no active viewers - not restarting (will resume when viewer connects)'
            );
          }
          return;
        }
        ...
        // Early generation: Pre-generate bumper if we're near the end of current file
        // This avoids race conditions where FFmpeg reaches the bumper before it's ready
        const currentFileDuration = mediaFiles[currentFileIndex].metadata?.duration || 0;
        const positionInFile = normalizedElapsed - accumulatedTime;
        const timeUntilNextFile = currentFileDuration - positionInFile;
        const includeBumpers = channel.config.includeBumpers !== false;

        // If we're within 30 seconds of transitioning to the next file, pre-generate its bumper
        // Use a threshold that gives us time to generate (~1 second) plus safety buffer
        const earlyGenerationThreshold = 30; // seconds
        if (timeUntilNextFile > 0 && timeUntilNextFile <= earlyGenerationThreshold && includeBumpers) {
          const nextFileIndex = (currentFileIndex + 1) % mediaFiles.length;
          const nextFile = mediaFiles[nextFileIndex];

          // Check if we've already pre-generated for this transition
          // Track using a key like "318->319" to avoid regenerating multiple times
          const tracker = this.concatProgressionTrackers.get(channelId);
          const transitionKey = `${currentFileIndex}->${nextFileIndex}`;

          if (nextFile && (!tracker?.lastPreGeneratedTransition || tracker.lastPreGeneratedTransition !== transitionKey)) {
            const outputDir = path.resolve(channel.config.outputDir);
            const bumperPath = this.concatFileManager.getBumperPath(outputDir);
              ...
              logger.info(
                {
                  ...
                  nextFile: nextFile.filename,
                  timeUntilTransition: timeUntilNextFile.toFixed(1),
                  threshold: earlyGenerationThreshold
                },
                'Early bumper generation: pre-generating bumper before file transition'
              );
              ...
              logger.info(
                {
                  ...
                  nextFile: nextFile.filename,
                  bumperPath,
                  note: 'Bumper pre-generated with buffer time before FFmpeg needs it'
                },
                'Early bumper generation complete'
              );

              // Mark this transition as pre-generated
              if (tracker) {
                tracker.lastPreGeneratedTransition = transitionKey;
              }
            } catch (error) {
              logger.warn(
                { channelId, error, nextFile: nextFile.filename },
                'Failed to pre-generate bumper (will retry on transition)'
              );
            }
          }
        }
        ...
        // NOTE: No longer advancing virtual time - position calculated on-demand from schedule_start_time
          ...
          if (nextFile && includeBumpers) {
            // Check if bumper was already pre-generated by early generation logic
            const tracker = this.concatProgressionTrackers.get(channelId);
            const transitionKey = `${currentFileIndex}->${nextFileIndex}`;
            const wasPreGenerated = tracker?.lastPreGeneratedTransition === transitionKey;
            ...
            if (wasPreGenerated) {
              logger.debug(
                {
                  ...
                  nextFile: nextFile.filename,
                  transitionKey,
                  note: 'Bumper already pre-generated by early generation, skipping fallback regeneration'
                },
                'Skipping fallback bumper generation (already pre-generated)'
              );
            } else {
              ...
              const bumperPath = this.concatFileManager.getBumperPath(outputDir);
              try {
                // Regenerate bumper with content for the next episode (fallback)
                // This is a fallback in case early generation didn't happen (e.g., short files, startup)
                // Normally, the bumper should already be generated by early generation logic above
                await this.bumperGenerator.generateBumperMP4(
                {
                  showName: nextFile.info.showName,
                  episodeName: nextFile.info.title || nextFile.getDisplayName(),
                ...
                logger.info(
                  {
                    ...
                    nextFile: nextFile.filename,
                    bumperPath,
                    note: 'Fallback bumper generation on transition (early generation may have missed this)'
                  },
                  'Regenerated bumper MP4 for next episode (fallback)'
                );
                
                // Mark this transition as generated (even though it was fallback)
                if (tracker) {
                  tracker.lastPreGeneratedTransition = transitionKey;
                }
              } catch (error) {
...
`;
   ...
   */
  private async hasActiveViewers(channelId: string): Promise<boolean> {
    try {
      const channel = await this.getChannel(channelId);
      return channel.getMetadata().viewerCount > 0;
    } catch {
      return false;
    }
  }
